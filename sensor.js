...
var config = {
  sensorboxID: 'WGWRKXFUAZQEIKYVBGGRSP9HYXLBWGLJ9JBXKOYDEY9VFQOBMCMNS9MO9JYKDDPBQPRHIUVVCHSBMJJYCECHLMGKQD',
  lat: '53.5586526',
  lon: '9.6476349'
}
...
function sendIOTA(sensor, value, cb) {
  ...
  iota.api.sendTransfer(seed, 4, 9, transfer, function(e, bundle) {
      if (e) cb("Error: " + e);
      var date = new Date();
      cb(date + "Successfully sent to IOTA (" + sensor + ")" + JSON.stringify(bundle[0].hash));
      if (value > 400) {
        changeLED(sensor + ': ' + value, 255, 55, 55);
      }
      else {
        changeLED(sensor + ': ' + value, 55, 255, 55);
      }
  })
    ...
    onInit: function (res) {
      if (res) {
        ...
        if (testOptions.airQuality) {
          ...
          // Analog Port 1
          // Air Quality Sensor
          console.log('AirQuality Analog Sensor..')
        ...
        if (testOptions.dhtDigital) {
          ...
          // Digital Port 2
          // DHT Sensor
          console.log('DHT Digital Sensor..')
        ...
        if (testOptions.loudnessAnalog) {
          ...
          //Analog Port 2
          // Loudness Sensor
          console.log('Loudness Analog Sensor..')
          loudnessSensor.start()
          setInterval(loudnessSensorGetAvgMax, 3000, loudnessSensor)
        }
      } else {
...
function loudnessSensorGetAvgMax(loudnessSensor) {
  ...
  //var val = grovepi.analogRead(loudness_sensor)
  sendIOTA("loudnessSensor", val.max, function (res) {
      console.log(res);
  });
